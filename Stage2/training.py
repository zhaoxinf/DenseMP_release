...
def main(_run, _config, _log):
    ...
    if isinstance(_config['eval_fold'], list):
        print(f'eval_fold: {_config["eval_fold"]} is list. Using the first value as the eval_fold.')
        assert len(_config['eval_fold']) == 1
        _config['eval_fold'] = _config['eval_fold'][0]
    elif isinstance(_config['eval_fold'], int):
        print(f'eval_fold: {_config["eval_fold"]} is int.')
    else:
        raise ValueError(f'eval_fold: {_config["eval_fold"]} is not int or list.')
    ...
    _log.info('###### Training ######')
    print('###### Training ######')
    print(f"print_interval: {_config['print_interval']}")
