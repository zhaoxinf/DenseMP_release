...
from .backbone.vit_backbones import ViTBasePatch16Encoder
# DEBUG
...
class FewShotSeg(nn.Module):
    ...
    def get_encoder(self, in_channels):
        assert in_channels==3, "ALPNet only supports RGB input"
        # 获取backbone
        if self.config['which_model'] == 'vit_base_patch16':
            # ViT
            self.encoder = ViTBasePatch16Encoder()
        else:
            # 不支持的backbone
