...
from .backbone.torchvision_backbones import TVDeeplabRes101Encoder, TVDeeplabRes101DenseCLEncoder, TVDeeplabRes101DenseCLEncoderPlus
from .backbone.mmcv_backbones import ResNet, SwinTransformer, VisionTransformer
...
class FewShotSeg(nn.Module):
    ...
    def __init__(self, in_channels=3, pretrained_path=None, cfg=None):
        ...
        if self.config['which_model'] == 'dlfcn_swin_mmcv':
            self.localconv = nn.Sequential(nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True), nn.Conv2d(1024, 256, kernel_size = 1, stride = 1, bias = False)) # reduce feature map dimension
        elif self.config['which_model'] == 'dlfcn_vit_mmcv':
            self.localconv = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True), nn.Conv2d(768, 256, kernel_size = 1, stride = 1, bias = False)) # reduce feature map dimension
        elif self.config['which_model'] == 'dlfcn_res50_mmcv':
            self.localconv = nn.Sequential(nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True), nn.Conv2d(2048, 256, kernel_size = 1, stride = 1, bias = False)) # reduce feature map dimension
        #! 这个是干嘛的?
        self.get_cls()
    ...
    def get_encoder(self, in_channels):
        ...
        if self.config['which_model'] == 'dlfcn_res101':
            if self.config['use_densecl_init']:
                if self.config['use_coco_init']:
                    self.encoder = TVDeeplabRes101DenseCLEncoderPlus()
                else:
                    self.encoder = TVDeeplabRes101DenseCLEncoder(pretrain=self.config['pretrained_backbone'])
            else:
                use_coco_init = self.config['use_coco_init'] # 是否使用coco预训练的Deeplab权重
                # 获取encoder
                self.encoder = TVDeeplabRes101Encoder(use_coco_init)
        elif self.config['which_model'] == 'dlfcn_swin_mmcv':
            self.encoder = SwinTransformer(arch='base', img_size=(256, 256), in_channels=in_channels)
        elif self.config['which_model'] == 'dlfcn_vit_mmcv':
            self.encoder = VisionTransformer(arch='base', img_size=(256, 256), in_channels=in_channels, out_type='featmap')
        elif self.config['which_model'] == 'dlfcn_res50_mmcv':
            self.encoder = ResNet(depth=50, in_channels=in_channels)
        elif self.config['which_model'] == 'dlfcn_res101_mmcv':
            self.encoder = ResNet(depth=101, in_channels=in_channels)
        else:
            # 不支持的backbone
        ...
        if self.pretrained_path:
            # 加载预训练模型权重
            if 'mmcv' in self.config['which_model']:
                # 使用mmcv模型时, 直接加载backbone的预训练权重
                # revise keys?
                load_checkpoint(self,
                                filename=self.pretrained_path,
                                map_location=torch.device("cpu"),
                                strict=False,
                                revise_keys=[['backbone.', 'encoder.']],)
            else:
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs, isval, val_wsize, show_viz = False, save_feature_map=False):
        """
        Args:
        ...
        if 'mmcv' in self.config['which_model']:
            img_fts = self.encoder(imgs_concat)[-1]
            # print(f'img_fts shape is {img_fts.shape}')
            img_fts = self.localconv(img_fts)
        else:
        ...
        if save_feature_map:
            qry_fts_copy = copy.deepcopy(qry_fts)
        ...
        if save_feature_map:
            return output, align_loss / sup_bsize, [bg_sim_maps, fg_sim_maps], assign_maps, qry_fts_copy
        else:
