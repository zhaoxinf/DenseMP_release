...
class TrainManualDataset(Dataset):
    """
    Dataset for validation
    ...
    """
    def __init__(self, dataset, train_classes: list, npart: int):
        super().__init__()
        self.dataset = dataset
        self.__curr_cls = None
        #self.test_classes = test_classes
    ...
    def set_curr_cls(self, curr_cls):
        #assert curr_cls in self.test_classes
        assert curr_cls in self.train_classes
        self.__curr_cls = curr_cls
    ...
    def __getitem__(self, idx):
        ...
        labelname = self.dataset.all_label_names[self.__curr_cls]
        # print(f"curr_cls: {self.__curr_cls}, labelname: {labelname}, scan_id: {sample['scan_id']}")
        if len(self.dataset.tp1_cls_map[labelname][sample['scan_id']])==0:
            sample["status"] = False
            sample["z_min"], sample["z_max"] = 0, 0
            sample["part_assign"] = 0
        else:
            sample["status"] = True
            z_min = min(self.dataset.tp1_cls_map[labelname][sample['scan_id']])
            z_max = max(self.dataset.tp1_cls_map[labelname][sample['scan_id']])
