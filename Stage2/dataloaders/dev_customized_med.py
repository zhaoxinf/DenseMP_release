...
from dataloaders.common import ReloadPairedDataset, ValidationDataset, TrainManualDataset, TrainMixDataset
from dataloaders.ManualAnnoDatasetv2 import ManualAnnoDataset
from dataloaders.MixDataset import MixDataset
...
def med_fewshot(dataset_name, base_dir, idx_split, mode, scan_per_load,
        transforms, act_labels, n_ways, n_shots, max_iters_per_load, min_fg = '', n_queries=1, fix_parent_len = None,
                exclude_list = [], **kwargs):
    """
    Dataset wrapper
    Args:
...
def med_fewshot_val(dataset_name, base_dir, idx_split, scan_per_load, act_labels, npart,
                    fix_length = None, nsup = 1, **kwargs):
    """
    ...
    """
    mydataset = ManualAnnoDataset(which_dataset = dataset_name, base_dir=base_dir, idx_split = idx_split,
                                  mode = 'val', scan_per_load = scan_per_load, transforms=None, min_fg = 1,
                                  fix_length = fix_length, nsup = nsup, **kwargs)
    mydataset.add_attrib('basic', attrib_basic, {})
...
def med_fewshot_train(dataset_name, base_dir, idx_split, transforms, scan_per_load, act_labels, npart,
                    fix_length = None, nsup = 1, **kwargs):
    """
    validation set for med images
    Args:
        ...
        idx_split:
            index of split
        transforms:
            same to the superpixel data augmentation
        scan_per_batch:
            number of scans to load into memory as the dataset is large
            use that together with reload_buffer
        act_labels:
    ...
    """
    mydataset = ManualAnnoDataset(which_dataset = dataset_name, base_dir=base_dir, idx_split = idx_split,
                                  mode = 'train', scan_per_load = scan_per_load, transforms=transforms, min_fg = 1,
                                  fix_length = fix_length, nsup = nsup, **kwargs)
    mydataset.add_attrib('basic', attrib_basic, {})

    trainset = TrainManualDataset(mydataset, train_classes = act_labels, npart = npart)

    return trainset, mydataset


def mix_fewshot_train(dataset_name, base_dir, idx_split, transforms, scan_per_load, act_labels, npart, nsup = 1, extern_normalize_func = None, exclude_list = [], superpix_scale= 'SMALL', fix_length = None,  **kwargs):
    """
    validation set for med images
    Args:
        dataset_name:
        ...
        nsup:  number of support scans, equivalent to nshot
    """
    mydataset = MixDataset(which_dataset = dataset_name, base_dir=base_dir, idx_split = idx_split, mode = 'train',transforms=transforms, scan_per_load = scan_per_load, min_fg = 1, fix_length = fix_length, nsup = nsup, exclude_list = exclude_list, extern_normalize_func = extern_normalize_func, superpix_scale = superpix_scale, **kwargs)
    mydataset.add_attrib('basic', attrib_basic, {})

    trainset = TrainMixDataset(mydataset, train_classes = act_labels, npart = npart)
