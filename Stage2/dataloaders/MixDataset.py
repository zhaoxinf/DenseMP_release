...
class MixDataset(BaseDataset):
    def __init__(self, which_dataset, base_dir, idx_split, mode, transforms,
                 scan_per_load, min_fg = '', fix_length = None, tile_z_dim = 3,
                 nsup = 1, exclude_list = [], extern_normalize_func = None,
                 num_rep = 2, superpix_scale = 'SMALL', **kwargs):
        """
        Manually labeled dataset
        Args:
        ...
        """
        super(MixDataset, self).__init__(base_dir)
        ...
        if len(exclude_list) > 0:
            print(f'###### Dataset: the following classes has been excluded {exclude_list} for manual dataset######')
        ...
        ######################## 根据superpixel和manual而不同 #########################################
        self.extern_normalize_func = extern_normalize_func
        # suppose manual label is on the first img
        self.nclass = len(self.real_label_name)
        self.img_lb_fids_manual = self.organize_sample_fids('manual')
        if self.extern_normalize_func is not None:  # helps to keep consistent between training and testing dataset.
            self.manual_norm_func = self.extern_normalize_func
            #print(f'###### Dataset: using external normalization statistics ######')
        else:
            self.manual_norm_func = get_normalize_op(self.img_modality,
                                              [fid_pair['img_fid'] for _, fid_pair in self.img_lb_fids.items()])
        #first suppose it is manual to load manual support image and query
        self.actual_dataset_manual = self.read_dataset_manual(self.img_lb_fids_manual, self.manual_norm_func)
        self.size_manual = len(self.actual_dataset_manual)
        #self.update_subclass_lookup()

        ######## superpixel
        self.img_lb_fids_superpixel = self.organize_sample_fids('superpixel')
        self.superpix_norm_func = get_normalize_op(self.img_modality,
                                                   [fid_pair['img_fid'] for _, fid_pair in self.img_lb_fids_superpixel.items()])
        self.actual_dataset_superpixel = self.read_dataset_superpixel(self.img_lb_fids_superpixel, self.superpix_norm_func)
        self.size_superpixel = len(self.actual_dataset_manual)
    ...
    def reload_buffer(self):
        ...
        del self.actual_dataset_manual
        del self.actual_dataset_superpixel
        ...
        print('does not implement yet')

    def organize_sample_fids(self, dataset):
        out_list = {}
    ...
    def read_dataset_manual(self, img_lb_fids, norm_func):
        ...
        for scan_id, itm in img_lb_fids.items():
            ...
            self.scan_z_idx_manual[scan_id] = [-1 for _ in range(img.shape[-1])]
            ...
            self.scan_z_idx_manual[scan_id][0] = glb_idx
            glb_idx += 1
            ...
            for ii in range(1, img.shape[-1] - 1):
                ...
                self.scan_z_idx_manual[scan_id][ii] = glb_idx
                glb_idx += 1
    ...
    def read_dataset_superpixel(self, img_lb_fids, norm_func):
        """
        Build index pointers to individual slices
        Also keep a look-up table from scan_id, slice to index
        """
    ...
    def __getitem__(self, index, choice=None):
        if choice is None:
            choice = np.random.choice(['manual', 'superpixel'], p=[0.25, 0.75])
        else:
            choice = choice

        if choice == 'manual':
            #print ("the img idx {} is manual labeled".format(index))
            self.nclass = len(self.real_label_name)
            self.fix_length = None
            self.update_subclass_lookup()

            index = index % len(self.actual_dataset_manual)
            curr_dict = self.actual_dataset_manual[index]
                ...
                if len(self.exclude_lbs) > 0:
                    for _ex_cls in self.exclude_lbs:
                        if curr_dict["z_id"] in self.tp1_cls_map[self.label_name[_ex_cls]][curr_dict["scan_id"]]: # this slice need to be excluded since it contains label which is supposed to be unseen
                            return self.__getitem__(index + torch.randint(low = 0, high = self.__len__() - 1, size = (1,)), choice='manual')
            ...
            #print ('this img idx {} is superpixel labeled'.format(index))
            self.nclass = len(self.pseu_label_name)
            self.fix_length = self.pseu_length

            index = index % len(self.actual_dataset_superpixel)
            curr_dict = self.actual_dataset_superpixel[index]

            sup_max_cls = curr_dict['sup_max_cls']
            if sup_max_cls < 1:
                return self.__getitem__(index + 1, choice = 'superpixel')
            ...
            for _ex_cls in self.exclude_lbs:
                if curr_dict["z_id"] in self.tp1_cls_map[self.real_label_name[_ex_cls]][curr_dict[
                    ...
                    return self.__getitem__(torch.randint(low=0, high=self.__len__() - 1, size=(1,)), choice = 'superpixel')
    ...
    def __len__(self):
        ...
        """
        if self.fix_length != None: # manual label fix length is none
            #assert self.fix_length >= len(self.actual_dataset)
            return self.fix_length
        else:  # superpixel label
            return len(self.actual_dataset_superpixel)
        ...
        for cls, dict_by_pid in self.overall_slice_by_cls.items():
            for pid, slice_list in dict_by_pid.items():
                if pid not in self.pid_curr_load:
                    continue
                self.idx_by_class[cls] += [ self.scan_z_idx_manual[pid][_sli] for _sli in slice_list ]
    ...
    def get_support(self, curr_class: int, class_idx: list, scan_idx: list, npart: int):
        ...
        for _part in range(npart):
            ...
            for scan_order in scan_idx:
                ...
                _glb_idx = self.scan_z_idx_manual[_scan_id][_zid]

                # almost copy-paste __getitem__ but no augmentation
                curr_dict = self.actual_dataset_manual[_glb_idx]
                img = curr_dict['img']
                lb = curr_dict['lb']
